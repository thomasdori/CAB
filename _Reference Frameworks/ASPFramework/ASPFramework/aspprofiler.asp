<%
' ASP Profiler v2.1
' Copyright © 2001-2003 Zafer Barutcuoglu. All Rights Reserved.
'
...
%>
  <basefont face="Arial">
  <font size=+3><b>ASP Profiler v2.1</b></font><br>
<%
End Sub
...
%>
  <font size=-1>
  Copyright © 2001-2003 Zafer Barutcuoglu. All Rights Reserved.<br>
...
sub FindFilePos(j, byref sFile, byref iFileLine)
   ...
   For i = 1 to <%=dictLimits.Count-1%>
      arr = k(i)
      If arr(0) > CInt(j) Then arr = k(i-1): Exit For
   Next
   sFile = arr(1)
...
function completeTable()
   ...
   If iTotalTime > 0 Then 
      divSummary.innerText = "Profile Time: " & iTotalTime & " cycles (" & CInt(iTotalTime * 1000 / 256) & " ms)"
      For i=0 To UBound(z.Items)
         r = z.Items(i)
...
function cmdDownloadTime_onclick()
   ...
   Set objHTTP = Nothing
   divDownloadTime.innerText = "Actual Time: " & t*256 & " cycles (" & CInt(t*1000) & " ms), " & size & " Bytes"
end function
-->
</SCRIPT>
</head>
